 ...
 * @returns {String}
 */
module.exports.decrypt = async (options) => {
    ...
    if (!await pathExists(encryptedFile)) throw `${encryptedFile} does not exist.`
    ...
    if (decryptedFile){
        ...
        await fs.writeFile(decryptedFile, decrypted, "utf-8")
    }
    return decrypted
}
 ...
 * @returns {Promise<>}
 */
module.exports.encrypt = async (options) => {
    ...
    if (!decryptedFile || ! await pathExists(decryptedFile)) throw `Error: ${decryptedFile} does not exist.`
    ...
    const output = require("fs").createWriteStream(encryptedFile)
    output.write(iv)


    const promise = new Promise((resolve, reject) => {
        require("fs").createReadStream(decryptedFile).pipe(cipher).pipe(output)
        output.on('finish', () => {
